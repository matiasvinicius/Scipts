...
<p> We'll create three models, the first with _am_ and the 4 variables 
with more correlation with mpg
<p> The second with _am_ and two variables with high correlation with _mpg_ 
and lower correlation between them (that is, explain mpg, but tries to reduce
the correlation between explanatory variables) 
<p> The third with all variables
```{r}
high_cor_fit <- lm(mpg ~ factor(am) + cyl + wt + disp + hp, data = mtcars)
some_high_cor_fit <- lm(mpg ~ factor(am) + cyl + wt, data = mtcars)
all_fit <- lm(mpg ~., data = mtcars)
```

<p> Doing the Analysis of Variance (ANOVA)
```{r}
anova(fit, all_fit, high_cor_fit, some_high_cor_fit)
```
